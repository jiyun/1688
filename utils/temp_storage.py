...
def save_resource_counts_temp(product_id: str, main_images: int, color_images: int,
    ...
    try:
        ...
        counts_file = os.path.join(TEMP_DIR, 'pending_counts.json')
...
def clear_pending_data():
    """清空临时数据文件"""
    files = [PENDING_RESOURCES_FILE, PENDING_PRICES_FILE, 
             os.path.join(TEMP_DIR, 'pending_counts.json')]
    for f in files:
        if os.path.exists(f):
            os.remove(f)
...
def has_pending_data() -> bool:
    ...
    return (os.path.exists(PENDING_RESOURCES_FILE) or 
            ...
            os.path.exists(os.path.join(TEMP_DIR, 'pending_counts.json')))
